...
{
    int power;
    constexpr Item(int power) : power(power) // constexpr생성자
    {
...
int main()
{
    ...
    int nums[value];     // 배열 선언에 value가 사용된 것으로 보아 컴파일 시간에 평가 되었다


    print<value>();     // 템플릿의 non type 파라메터에 value가 사용된 것으로 보아 컴파일 시간에 평가 되었다.
